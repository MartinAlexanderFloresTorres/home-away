      ...
      {isOpen && (
        <div className='absolute rounded-xl shadow-sm w-[40vw] md:w-3/4 bg-white overflow-hidden right-0 top-12 text-sm z-10'>
          <div className='flex flex-col cursor-pointer'>
            {currentUser ? (
              <>
                <MenuItem label='Mis viajes' onClick={() => router.push('/viajes')} />
                <MenuItem label='Mis favoritos' onClick={() => router.push('/favoritos')} />
                <MenuItem label='Mis reservaciones' onClick={() => router.push('/reservaciones')} />
                <MenuItem label='Mis propiedades' onClick={() => router.push('/propiedades')} />
                <MenuItem label='Home Away una propiedad' onClick={rentModal.onOpen} />
                <hr />
                <MenuItem label='Cuenta' onClick={() => router.push('/perfil')} />
                <MenuItem label='Cerrar sesión' onClick={() => signOut()} />
              </>
            ) : (
              <>
                <MenuItem label='Iniciar sesión' onClick={loginModal.onOpen} />
                <MenuItem label='Regístrate' onClick={registerModal.onOpen} />
              </>
            )}
          </div>
        </div>
